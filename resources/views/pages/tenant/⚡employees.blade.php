...
new class extends Component {
    ...
    // إعادة تحميل الموظفين والفروع والأدوار فور تغيير المتجر النشط
    #[On('tenant-changed')]
    public function loadData()
    {
        ...
        if ($tenantId) {
            // جلب الموظفين التابعين للمتجر النشط المحدد في الجلسة
    ...
    public function edit($id)
    {
        ...
        $employee = User::where('tenant_id', $tenantId)->where('is_owner', false)->findOrFail($id);
    ...
    public function save()
    {
        ...
        if (!$tenantId) {
            session()->flash('message', 'يرجى اختيار متجر أولاً لتتمكن من إضافة الموظف.');
            return;
        }
    ...
    public function delete($id)
    {
        ...
        User::where('tenant_id', $tenantId)->where('is_owner', false)->findOrFail($id)->delete();
    ...
    <div>
        <div
            ...
            <div>
                ...
                <flux:subheading>إضافة الموظفين وتعيين فروعهم وأدوارهم الوظيفية للمتجر الحالي</flux:subheading>
            </div>
            <div>
                    ...
                    @empty
                        <flux:table.row>
                            <flux:table.cell colspan="6" align="center" class="py-8 text-zinc-500">
                                لا يوجد موظفون مضافين لهذا المتجر بعد.
                            </flux:table.cell>
                        </flux:table.row>
                    @endforelse
                </flux:table.rows>
