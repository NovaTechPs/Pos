    ...
    $this->selectedCustomerForStatement = Customer::where('tenant_id', $tenantId)
        ->with([
            ...
            'payments' => function($query) {
                $query->select('id', 'customer_id', 'amount', 'created_at', 'notes');
            }
        ])
        ->findOrFail($id);
...
@if ($showStatementModal && $selectedCustomerForStatement)
    @php
        ...
        // إضافة الطلبات (مدين)
        foreach ($selectedCustomerForStatement->orders as $order) {
            $transactions->push([
        ...
        // إضافة المدفوعات (دائن)
        foreach ($selectedCustomerForStatement->payments as $payment) {
            $transactions->push([
        ...
        // حساب الرصيد الحالي المتراكم
        $runningBalance = $selectedCustomerForStatement->opening_balance;
    @endphp
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl my-auto overflow-hidden border border-gray-100 max-h-[90vh] flex flex-col">
            ...
            <div class="p-4 sm:p-6 space-y-4 overflow-y-auto flex-1" id="statement-print-area">
                <div class="grid grid-cols-2 gap-2 text-xs sm:text-sm bg-gray-50 p-3 rounded-lg">
                    <div>الرصيد الافتتاحي: <span class="font-bold">{{ number_format($selectedCustomerForStatement->opening_balance, 2) }}</span></div>
                </div>
                    ...
                    <table class="w-full text-right text-xs min-w-[500px]">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            @foreach ($sortedTransactions as $item)
                                ...
                                @endphp
                                <tr>
                                    <td class="p-2 whitespace-nowrap">{{ $item['date']->format('Y-m-d H:i') }}</td>
                                    <td class="p-2">{{ $item['description'] }}</td>
                                    <td class="p-2 whitespace-nowrap text-red-600">{{ $item['debit'] > 0 ? number_format($item['debit'], 2) : '-' }}</td>
                                    <td class="p-2 whitespace-nowrap text-emerald-600">{{ $item['credit'] > 0 ? number_format($item['credit'], 2) : '-' }}</td>
                                    <td class="p-2 whitespace-nowrap font-semibold">{{ number_format($runningBalance, 2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
